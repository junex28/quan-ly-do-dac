...
dojo.provide("com.esri.solutions.jsviewer.widgets.EditSearchWidget");
...
dojo.declare(
	"com.esri.solutions.jsviewer.widgets.EditSearchWidget",
	[com.esri.solutions.jsviewer._BaseWidget, com.esri.solutions.jsviewer._MapGraphicsMaintainer],
	{
	    ...
	    fetchRecords: function(layerId, objectIds) {
	        try {
	            ...
	            this.rowPerPage = this.layers[layerId].rowPerPage;
	    ...
	    queryRecordsByPage: function(layerId, pageNumber) {
	        try {
	            ...
	            this.featureLayer = new esri.layers.FeatureLayer(this.layers[layerId].url, {
	                outFields: this.layers[layerId].fields,
	                mode: esri.layers.FeatureLayer.MODE_SELECTION
	            });
	            ...
	            query.outFields = this.layers[layerId].fields;
	            //console.log(JSON.stringify(query));            

	            // Query for the records with the given object IDs and populate the grid
	            this.featureLayer.selectFeatures(query, esri.layers.FeatureLayer.SELECTION_NEW, dojo.hitch(this, function(features) {
	    ...
	    updateGrid: function(layerId, features, pageNumber) {
	        ...
	        try {
	            ...
	            var ce = this.configData.search.layerInfo;
	            console.log(features);

	            dojo.forEach(features, dojo.hitch(this, function(f) {
	                var layer = this.layers[layerId];
	                console.log(this.layers[layerId].fields);
	    ...
	    onResultClick: function(evt) {
	        ...
	        if (evt.resultItem) {
	            dojo.publish("widgetEditAttributeEvent", [evt.resultItem, evt.resultItem.graphic, evt.resultItem.location, true, evt.resultItem.zoomScale]);
	        }
	        else {
	            dojo.publish("widgetEditAttributeEvent", [null]);
	        }
	    ...
	    onResultHover: function(evt) {
	        ...
	        // evt.resultItem.location is the place to zoom to
	        if (evt.resultItem) {
	            dojo.publish("widgetEditAttributeEvent", [evt.resultItem, evt.resultItem.graphic, evt.resultItem.location, false]);
	        }
	        else {
	            dojo.publish("widgetEditAttributeEvent", [null]);
	        }
	            ...
	            if (geometry) {
	                try {
	                    ...
	                    params.bufferSpatialReference = new esri.SpatialReference({
	                        wkid: this.configData.search.bufferWKID
	                    });
