...
dojo.provide("com.esri.solutions.jsviewer.widgets.LiveLayerWidget");
...
dojo.requireLocalization("com.esri.solutions.jsviewer.widgets", "LiveLayerWidgetStrings");

//dijits used in template
dojo.require("com.esri.solutions.jsviewer.ResultList");

dojo.declare(
	"com.esri.solutions.jsviewer.widgets.LiveLayerWidget",
	[com.esri.solutions.jsviewer._BaseWidget, com.esri.solutions.jsviewer._MapGraphicsMaintainer],
	{
		...
		templatePath:  dojo.moduleUrl("com.esri.solutions.jsviewer", "widgets/templates/LiveLayerWidget.html"),
	    ...
	    postMixInProperties: function() {
			console.log("LiveLayerWidget postMixInProperties");
			try {
			    //super
				...
				// Init i18n
				this.i18nStrings = dojo.i18n.getLocalization("com.esri.solutions.jsviewer.widgets", "LiveLayerWidgetStrings");
			}
			catch (err) { console.error(err); }		
		},
		
		postCreate: function(){
			console.log("LiveLayerWidget postCreate");
		    try{
		        //super
		        this.inherited(arguments);
		...
		startup: function(){
			console.log("LiveLayerWidget startup");
		    //super
		    this.inherited(arguments);
		...
		queryCallback: function(featureSet) {
			if (featureSet && featureSet.features) {
				...
				for (var i = 0; i < featureSet.features.length; i++) {
					...
					var link = null;
					var content = "<table>";
					for (var fIdx in this.queryFields) {
						var field = this.queryFields[fIdx];
						if (field === this.linkField) {
							link = f.attributes[field];
						}
						else if (field !== this.titleField) {
